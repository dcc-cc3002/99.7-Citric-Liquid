...
public class PlayerTest {
  ...
  // TEST PARA NUEVOS METODOS.
  @Test
  public void isAtakedTest() {
    int hp = suguri.getCurrentHp();
    suguri.isAttacked(2);
    Assertions.assertEquals(hp - 2, suguri.getCurrentHp());
    suguri.isAttacked(100);
    Assertions.assertEquals(0, suguri.getCurrentHp());
  }

  /**
   * test the consistency of attack_point.
   *
   */
  @RepeatedTest(100)
  public void attackPointTest() {
    Assertions.assertTrue(suguri.attack_point() <= suguri.getAtk() + 6);
    Assertions.assertTrue(suguri.attack_point() > 0);

  }

  @Test
  public void chapterTest() {
    Assertions.assertEquals(suguri.getChapters(), 0);
    suguri.setChapters(1);
    Assertions.assertEquals(suguri.getChapters(), 1);
  }

  @Test
  public void panelTest() {
    Assertions.assertNull(suguri.getActualPanel());
    suguri.setActualPanel(new NeutralPanel());
    Assertions.assertNotNull(suguri.getActualPanel());
    Assertions.assertEquals(NeutralPanel.class, suguri.getActualPanel().getClass());
